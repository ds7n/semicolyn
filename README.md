...
- **First-host onboarding flow** — first-launch empty state + targeted tooltips for Glymr-specific concepts; surfaced after dropping ssh_config import
- **Settings sub-screen layouts** — App preferences, Security, About contents
- **Pro / paid version scope** — stance is no foundational functionality behind paid; needs formal scope for what qualifies as Pro
- **Keyboard / input UX (remaining)** — v2 custom inputView and its letter-to-alt mapping
- **Connection-status banner expanded view** — tap-to-expand latency / mosh frame counts / roam history
- iPad navigation, layout templates, external keyboard, monetization
