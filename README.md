...
| **3 — Terminal + tmux** | `tmux -CC` parser, session/pane model, command encoder, controller, Rust transport (all done²); **Plan A done**: connect-time `tmux -V` probe → attach `tmux -CC` and render the **active pane** end-to-end (output + `send-keys`), else degrade to raw PTY with an amber banner; multi-pane/multi-window render (Plan B), UX polish (C), context-SM + crash banner (D) pending | ◐ Attach + single-pane live; multi-pane pending |
