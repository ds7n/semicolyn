...
final class ResolutionTests: XCTestCase {
    ...
    func testNestedConfigLeavesResolve() {
        ...
        XCTAssertTrue(resolveUseTmux(host: host(), defaults: Defaults()))        // builtin true
        XCTAssertFalse(resolveUseTmux(
            host: host { $0.semicolyn = .explicit(SemicolynConfig(tmux: TmuxConfig(useTmux: false))) },
            defaults: Defaults()))
    }
    ...
    func testHostSetsOneLeafStillInheritsDefaultsTmuxLeaf() {
        // Host sets ONLY osc52.allow; Defaults sets tmux.useTmux = false.
        let h = host {
        ...
        XCTAssertFalse(resolveUseTmux(host: h, defaults: d),
    ...
    func testTmuxSessionNameLeafIndependence() {
        // Host sets ONLY sessionName; Defaults sets ONLY useTmux=false.
