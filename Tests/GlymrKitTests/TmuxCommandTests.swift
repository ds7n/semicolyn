...
final class TmuxCommandTests: XCTestCase {
    ...
    func testKillSessionValidGlymrName() {
        XCTAssertEqual(TmuxCommand.killSession(name: "glymr-a3f7c2e9"),
                       "kill-session -t glymr-a3f7c2e9")
    }

    func testKillSessionValidAltName() {
        XCTAssertEqual(TmuxCommand.killSession(name: "glymr-a3f7c2e9-1b4d"),
                       "kill-session -t glymr-a3f7c2e9-1b4d")
    }
    ...
    func testKillSessionRejectsWhitespace() { // injection vector: arg split
        XCTAssertNil(TmuxCommand.killSession(name: "glymr foo"))
    }
