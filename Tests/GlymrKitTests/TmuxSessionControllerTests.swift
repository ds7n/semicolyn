...
final class TmuxSessionControllerTests: XCTestCase {
    ...
    func testStartReturnsExecCreateOrAttachString() {
        ...
        XCTAssertEqual(c.start(sessionName: "glymr-a3f7c2e9"),
                       "tmux -CC new-session -A -s glymr-a3f7c2e9")
    ...
    func testStartIsOnceOnly() {
        let c = TmuxSessionController()
        _ = c.start(sessionName: "glymr-a3f7c2e9")
        XCTAssertNil(c.start(sessionName: "glymr-a3f7c2e9"), "second start must be refused")
    }

    func testSessionChangedTransitionsToAttached() {
        let c = TmuxSessionController()
        _ = c.start(sessionName: "glymr-a3f7c2e9")
        let out = c.feed(bytes("%session-changed $7 glymr-a3f7c2e9\n"))
        ...
        XCTAssertEqual(c.state.sessionName, "glymr-a3f7c2e9")
    }

    func testExitTransitionsToExitedWithReason() {
        let c = TmuxSessionController()
        _ = c.start(sessionName: "glymr-a3f7c2e9")
        _ = c.feed(bytes("%session-changed $7 glymr-a3f7c2e9\n"))
    ...
    private func attachedController() -> TmuxSessionController {
        let c = TmuxSessionController()
        _ = c.start(sessionName: "glymr-a3f7c2e9")
        _ = c.feed(Array("%session-changed $7 glymr-a3f7c2e9\n".utf8))
        return c
    }
}
