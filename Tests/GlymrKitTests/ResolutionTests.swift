...
final class ResolutionTests: XCTestCase {
    // `GlymrKit.Host` is qualified to avoid colliding with Foundation's `Host`
    // class, which is in scope transitively on Linux.
    private func host(_ b: (inout GlymrKit.Host) -> Void = { _ in }) -> GlymrKit.Host {
        var h = GlymrKit.Host(id: UUID(), label: "l", hostName: "h"); b(&h); return h
    }
    ...
    func testNestedConfigLeavesResolve() {
        ...
        XCTAssertFalse(resolveTmuxAttemptControlMode(
            host: host { $0.glymr = .explicit(GlymrConfig(tmux: TmuxConfig(attemptControlMode: false))) },
            defaults: Defaults()))
    }
}
