...
final class EncryptedRecordStoreTests: XCTestCase {
    ...
    private func host() -> NeotildeKit.Host {
        NeotildeKit.Host(id: UUID(), label: "prod", hostName: "db.internal", user: .explicit("deploy"))
    }
    ...
    func testRoundTripAndList() throws {
        ...
        XCTAssertEqual(try store.get(.host, id: h.id, as: NeotildeKit.Host.self), h)
        XCTAssertNil(try store.get(.host, id: UUID(), as: NeotildeKit.Host.self))    // absent → nil
        XCTAssertEqual(try store.list(.host, as: NeotildeKit.Host.self).map(\.value), [h])
        try store.delete(.host, id: h.id)
        XCTAssertNil(try store.get(.host, id: h.id, as: NeotildeKit.Host.self))
    }
    ...
    func testBackendHoldsCiphertextNotPlaintext() throws {
        ...
        XCTAssertThrowsError(try JSONDecoder().decode(NeotildeKit.Host.self, from: raw))
    }
    ...
    func testWrongKeyFails() throws {
        ...
        XCTAssertThrowsError(try attacker.get(.host, id: h.id, as: NeotildeKit.Host.self)) {
    ...
    func testTamperedBlobFails() throws {
        ...
        XCTAssertThrowsError(try EncryptedRecordStore(backend: backend, key: key)
            .get(.host, id: h.id, as: NeotildeKit.Host.self)) {
            XCTAssertEqual($0 as? RecordEnvelopeError, .decryptionFailed)
        }
    }
}
