...
final class RecordEnvelopeTests: XCTestCase {
    ...
    func testSealThenOpenRoundTripsRecord() throws {
        ...
        // Qualify as NeotildeKit.Host: on Linux, swift-corelibs-Foundation also
        // exports a `Host` type, ambiguous in type-annotation position.
        let opened: NeotildeKit.Host = try RecordEnvelope.open(sealed, as: NeotildeKit.Host.self, key: key)
    ...
    func testWrongKeyFailsToOpen() throws {
        ...
        XCTAssertThrowsError(
            try RecordEnvelope.open(sealed, as: NeotildeKit.Host.self,
    ...
    func testTamperedCiphertextFailsToOpen() throws {
        ...
        XCTAssertThrowsError(
            try RecordEnvelope.open(sealed, as: NeotildeKit.Host.self, key: key)
        ) { error in
            XCTAssertEqual(error as? RecordEnvelopeError, .decryptionFailed)
        }
    }
    ...
    func testMalformedBlobFailsToOpenWithTypedError() throws {
        ...
        XCTAssertThrowsError(
            try RecordEnvelope.open(garbage, as: NeotildeKit.Host.self, key: key)
        ) { error in
            XCTAssertEqual(error as? RecordEnvelopeError, .decryptionFailed)
        }
    }
