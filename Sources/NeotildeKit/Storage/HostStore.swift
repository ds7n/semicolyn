...
public struct HostStore {
    ...
    @discardableResult
    public func saveHost(_ host: NeotildeKit.Host) throws -> SaveOutcome {
    ...
    public func host(id: UUID) throws -> NeotildeKit.Host? {
        try records.get(.host, id: id, as: NeotildeKit.Host.self)
    }

    public func allHosts() throws -> [NeotildeKit.Host] {
        try records.list(.host, as: NeotildeKit.Host.self).map(\.value)
    }
