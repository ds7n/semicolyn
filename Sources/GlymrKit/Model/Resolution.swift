...
// MARK: Glymr-extension nested-config leaves
...
public func resolvePredictorIncognito(host: Host, defaults: Defaults) -> Bool {
    resolveOptional(host.glymr, defaults.glymr)?.predictor?.incognito ?? false
}
public func resolveTmuxAttemptControlMode(host: Host, defaults: Defaults) -> Bool {
    resolveOptional(host.glymr, defaults.glymr)?.tmux?.attemptControlMode ?? true
}
