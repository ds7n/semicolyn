...
public struct HostStore {
    ...
    @discardableResult
    public func saveHost(_ host: GlymrKit.Host) throws -> SaveOutcome {
    ...
    public func host(id: UUID) throws -> GlymrKit.Host? {
        try records.get(.host, id: id, as: GlymrKit.Host.self)
    }

    public func allHosts() throws -> [GlymrKit.Host] {
        try records.list(.host, as: GlymrKit.Host.self).map(\.value)
    }
