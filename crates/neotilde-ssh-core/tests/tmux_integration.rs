...
//! produces a control-mode handshake. Gated on `NEOTILDE_TEST_SSHD`.
...
async fn exec_streams_command_stdout() {
    ...
    let _session = conn
        .open_exec(
            "printf neotilde-exec-marker".into(),
            "xterm".into(),
            80,
            24,
            Arc::new(col.clone()),
        ...
        .await
        .expect("open exec");
    let saw = wait_until(|| col.text().contains("neotilde-exec-marker")).await;
    assert!(
        saw,
...
async fn tmux_control_mode_emits_handshake() {
    ...
    let session = conn
        .open_exec(
            "tmux -CC new-session -A -s neotilde-test".into(),
            "xterm".into(),
            80,
            24,
            Arc::new(col.clone()),
    ...
    // Both markers must be on the SAME line (`%session-changed $N neotilde-test`) —
    ...
    let saw = wait_until(|| {
        col.text()
            .lines()
            .any(|l| l.contains("%session-changed") && l.contains("neotilde-test"))
    })
    .await;
    assert!(
        saw,
...
async fn tmux_cc_new_session_produces_control_mode_handshake() {
    ...
    let session = conn
        .open_exec(
            "tmux -CC new-session -A -s neotilde-itest".into(),
            "xterm-256color".into(),
            80,
            24,
            Arc::new(col.clone()),
