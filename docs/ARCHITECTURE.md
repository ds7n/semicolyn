...
The design keeps the **Apple-only UI/SDK layer thin** so the maximum surface stays Linux-testable. Decision logic lives in `Sources/NeotildeKit/` (pure, XCTest on the Linux fast loop); the Rust SSH core is exposed through a UniFFI XCFramework; only SwiftUI/SwiftTerm/Keychain/CloudKit wiring is Apple-gated. Crypto goes through swift-crypto on Linux and system CryptoKit on Apple behind `#if canImport(CryptoKit)`. (The two-tier rule and its build/test consequences: [CLAUDE.md](../CLAUDE.md).)
...
| `Sources/NeotildeKit/` | Platform-agnostic Swift: `Model/`, `Storage/`, `Crypto/`, `Tmux/`, `Predictor/`, `Theme/`, `Terminal/`, `IO/` |
