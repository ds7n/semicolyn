# System-wide iOS custom keyboard extension powered by Glymr's predictor
...
Extract the on-device predictor engine (CMS + Bloom sketches, per-prefix gating, seed deference) from Glymr and ship it as a standalone iOS keyboard extension. Users get Glymr's "learns my vocabulary, never silently rewrites it" suggestion behavior in any app — Messages, Notes, email, code editors.
...
- Could be a separate paid app or a Glymr Pro perk — natural expansion of the same engine investment.
...
- **Data isolation:** does the system keyboard share its sketch with the Glymr app (App Group container), or are they independent vocabularies? Shared = better suggestions in the terminal too; independent = cleaner privacy story.
- **Seed choice:** the terminal-flavored seed (carapace + tldr + dotfiles) is wrong here. Need a general English / code-flavored seed.
- **Settings surface:** how the user configures it from outside the parent Glymr app — extensions can't show arbitrary UI.
- **Monetization:** standalone app, in-app purchase, bundled with Glymr, "Glymr Pro" tier.
- **Privacy review:** keyboard extensions are scrutinized — the transparency story has to be airtight from day one.

## Relationship to Glymr v1

This is **not** v1 scope. It's a "if the predictor engine proves out in Glymr, here's a natural way to ship it more broadly" idea. Revisit after v1 ships and there's real predictor performance data.
