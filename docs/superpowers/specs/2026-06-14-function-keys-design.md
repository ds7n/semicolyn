...
iPhone has no native F-key access at all. Apps that depend on F-keys — `htop`, `top`, `mc`, various TUI debuggers, occasional vim mappings, custom user setups — are otherwise unusable from Semicolyn. The design must surface F-keys cleanly for users who need them while staying out of the way for the much larger set of users who never touch them.
