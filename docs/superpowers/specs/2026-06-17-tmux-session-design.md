...
Define how Neotilde names its tmux sessions and what happens when the same user signs into the same Unix account on the same host from multiple iCloud-paired iOS devices.

## Naming

Sessions are named `neotilde-<accountHash>` where `accountHash` is a stable 8-character lowercase hex derived from the user's iCloud account identity.
...
- Truncated to 8 chars: collision probability with another `neotilde-*` session on the same Unix user is negligible in practice (8 hex = 2^32 space; you'd need ~65k other Neotilde users sharing this Unix account to hit a 50% birthday-paradox collision).

**Examples.** `neotilde-a3f7c2e9`, `neotilde-1b4d8201`. The prefix `neotilde-` makes the session obviously Neotilde-owned for anyone inspecting `tmux ls` server-side.

## Default behavior: shared session per Apple ID

When the user connects to a host from device A and the session `neotilde-<accountHash>` does not exist server-side, Neotilde creates it via `tmux new-session -d -s neotilde-<accountHash>` and attaches via control mode.

When the user later connects from device B (same Apple ID), Neotilde sees the session exists and attaches via `tmux -CC attach-session -t neotilde-<accountHash>`. **Both clients are now attached to the same session.**
...
The Esc-pill picker entry maps to a (host, session) pair, not to a (host) alone. In the default case there's one entry per host because every connection targets the shared `neotilde-<accountHash>` session.
...
> **End session on `<host>`?**
> Closes Neotilde's connection AND kills the tmux session server-side.
> Any windows, panes, and running processes inside the session are terminated.
>
> [ End session ] ← red
> [ Cancel ]

On confirm, Neotilde runs `tmux kill-session -t neotilde-<accountHash>` (or the alt-N variant for non-default sessions) before closing the transport. If the user has other devices currently attached to the same session, they're booted with a banner: *"Session ended from another device."*
...
On invoke, Neotilde opens a new connection to the same host using a one-off session name: `neotilde-<accountHash>-<short-uuid>` where `<short-uuid>` is a 4-character hex tag. A new picker entry appears labeled `<host-label> · alt N`. The new session is independent: no cross-device sharing with the default session, no cross-device sharing with other alt sessions.
...
- **Concurrent first-connect from two devices.** Device A and device B both try to create `neotilde-<accountHash>` simultaneously. tmux's `new-session -d` is atomic; the second one will see the session already exists and attach instead. No race condition surface beyond "second device picks up an empty fresh session."
...
- **User has `tmux` set up with a custom `default-shell`, hook scripts, or other `.tmux.conf` config.** Neotilde respects it — `tmux new-session -d -s <name>` invokes the user's config normally. No override.
...
- **Not a session manager.** Neotilde doesn't list all tmux sessions on the host; it only knows about the ones it created (encoded in the picker entries). A user who wants to attach to a hand-created session uses `tmux attach -t othername` from raw SSH, not Neotilde.
...
- [[2026-06-16-first-host-onboarding-design]] — Tips & Gestures gets one new sentence: *"When you're signed into the same Apple ID, Neotilde shares one tmux session per host across your iPhone and iPad. Start vim on iPad, switch to iPhone, keep typing."*
