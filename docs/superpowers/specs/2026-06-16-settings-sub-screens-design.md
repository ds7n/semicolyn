...
second prompt for Glymr by default would be inconsistent with iOS norms
...
- **Toggle, off by default.** When off, Glymr opens directly after device
  unlock. When on, Glymr shows a Face ID prompt before revealing the
...
- **Active sessions on re-lock** are **hidden, not killed.** Live SSH and
  ...
  point of mosh.
- The lock view itself is a full-screen sheet with the Glymr mark, a
...
- **No grace period for failed Face ID.** Fall through to the device
  ...
  cancels stays on the lock view — Glymr does not back out of itself.
...
- **Appearance** — Glymr ships with one palette (cool-dark + bell-bronze).
...
- Link to the public privacy page on glymr's site (URL TBD before
...
- **To:** the support address (TBD before shipping; placeholder until
  then).
- **Subject:** `Glymr feedback — 1.0.0 (1234)` (version pre-filled).
...
Read-only row at the bottom. Format `Glymr <semver> (<build>)`. **Tap to
copy** the full string to the clipboard (handy for bug reports). Light
haptic on copy, no visible toast.

### Cut from About & Help

- **Terms of service** — Glymr has no user account, no service contract.
