...
The pill carries the literal label **"Esc"** plus a small dim bronze `≡` glyph in the corner — the same hint-glyph pattern used elsewhere in Semicolyn to signal "this slot has more gestures." Window title is *not* shown on the pill itself; the user identifies the current window by terminal content or by opening a picker.
