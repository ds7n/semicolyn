...
The shipped app (`SemicolynKit`) must not carry a tldr/markdown parser. So the
build-time code lives in its own targets:

```
Sources/SeedKit/            library, depends on SemicolynKit — pure, unit-tested
...
Sources/semicolyn-seedbuild/    executable, depends on SeedKit — thin I/O glue
...
## semicolyn-seedbuild — thin executable

`semicolyn-seedbuild <pages-dir> <out-dir>`: recursively find `*.md` under
