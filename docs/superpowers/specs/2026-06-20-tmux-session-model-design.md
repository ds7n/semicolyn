...
Maintain the in-memory *structure* of the single tmux session Semicolyn is attached
...
**Swift, in `Sources/SemicolynKit/Tmux/`.** A **value-type `struct TmuxSessionState`
...
- **One session per connection** (per [[2026-06-17-tmux-session-design]]: Semicolyn
  attaches to one `semicolyn-<accountHash>`). The model represents that single
...
- [[2026-06-17-tmux-session-design]] — the single attached session this model
  represents is the `semicolyn-<accountHash>` session named there.
