...
Skipping this in v1 cedes the corporate / managed-SSH user segment to Termius. Semicolyn's positioning as a security-first, professional SSH client warrants including it.

## Scope

- Client-side cert presentation only. Semicolyn does **not** issue, generate, or sign certificates; that's the user's CA's job.
...
- The "Create new" tab does not gain a cert field — Semicolyn-generated keys are not certified by a CA. (User can sign the generated public key with their CA out-of-band and re-import via "Import existing.")
...
- **Cert present + not expired:** Semicolyn passes `<cert> + <private-key>` to the SSH stack. The stack negotiates a cert-based key exchange (algorithm name `ssh-ed25519-cert-v01@openssh.com`, etc.).
- **Cert present + expired:** Semicolyn refuses to use the identity for this connection. The connect attempt fails with a clear error: *"Certificate expired on `<identity>` ({date}). Re-import or remove the certificate to use the underlying key instead."* No silent fallback to the bare key — falling back would surprise the user (their CA-signed identity suddenly auths as a different user).
...
**CA trust:** Semicolyn does not validate the CA against any client-side trust store. The server is the only party that decides if a cert is valid (it has the CA's trusted public key in `TrustedUserCAKeys`). Client-side cert validation would duplicate the server's job without benefit. Semicolyn only checks that:
...
- **Cert generation from Semicolyn.** Semicolyn does not sign. Users get certs from their CA out-of-band.
