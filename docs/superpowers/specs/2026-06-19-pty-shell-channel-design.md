...
|---|---|---|
| shell echoes input | `open_shell`, `write("echo semicolyn-marker\n")` | accumulated output contains `semicolyn-marker` (poll ≤ a few s, else fail) |
...
All tests are gated behind `SEMICOLYN_TEST_SSHD` like the existing auth/connect
suites. The current Alpine `sshd` fixture already provides `/bin/sh` and `stty`,
so no fixture changes are required.

## Exit criteria

- `cargo test -p semicolyn-ssh-core` (unit + connect + auth + the new shell suite)
