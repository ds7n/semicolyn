...
**Architecture:** The spec's save-time validation rules become a pure, Linux-tested `HostFormValidation` over a `HostDraft` value type in `GlymrKit` (cycle, duplicate-label, required fields, no-user, inline-jumphost, port-forward, stale passwordRef). The SwiftUI editor binds to that validator; the list and editor read/write through `AppStores` (Plan 1). Connect-from-saved resolves the host's effective config via the Phase-2a resolution table and reuses Plan 1's `TofuHostKeyVerifier`.

**Tech Stack:** Swift 6 (`GlymrKit`, the validation core — Linux-tested), Swift 5 app target (SwiftUI), XCTest, the Phase-2a storage core + Plan-1 trust wiring. Linux loop: `docker compose run --rm dev swift test`. App target compiles only in the macOS CI job.
...
- Placement: validation logic in `Sources/GlymrKit/Model/`, Linux tests in `Tests/GlymrKitTests/`; app UI in `App/`.
- **GlymrKit stays Linux-clean.** The validation core has no SwiftUI/Apple deps and is fully Linux-tested. SwiftUI (`App/`) compiles only in macOS CI; absence of XCTest for views is expected, not a defect.
...
**Files:**
- Create: `Sources/GlymrKit/Model/HostFormValidation.swift`
- Test: `Tests/GlymrKitTests/HostFormValidationTests.swift`
...
```swift
// Tests/GlymrKitTests/HostFormValidationTests.swift
...
- Modify: `App/GlymrApp.swift` (root → `HostListView`)
...
- [ ] **Step 3:** Point `GlymrApp` root at `HostListView`.
...
- Test: `Tests/GlymrKitTests/HostEditorBindingTests.swift` (only if a Linux-testable binding helper is extracted; otherwise none)
...
### Task 5: Host editor — Mosh + Tailscale + Glymr + Delete (App)
...
- Section **Glymr behavior**: `glymr.predictor.incognito`, `glymr.tmux.attemptControlMode` toggles.
...
- [ ] **Step 4:** Commit — `feat: host editor — Mosh, Tailscale, Glymr sections + delete`
