...
- Placement: Swift in `Sources/SemicolynKit/Tmux/`. No Apple-only APIs (must compile + test on Linux). No `Observation`/SwiftUI — pure value type.
...
**Files:**
- Modify: `Sources/SemicolynKit/Tmux/PaneLayout.swift` (append an extension)
- Test: `Tests/SemicolynKitTests/PaneLayoutPanesTests.swift`
...
```swift
// Tests/SemicolynKitTests/PaneLayoutPanesTests.swift
...
Append to `Sources/SemicolynKit/Tmux/PaneLayout.swift`:
...
```bash
git add Sources/SemicolynKit/Tmux/PaneLayout.swift Tests/SemicolynKitTests/PaneLayoutPanesTests.swift
...
**Files:**
- Create: `Sources/SemicolynKit/Tmux/TmuxSessionState.swift`
- Test: `Tests/SemicolynKitTests/TmuxSessionStateTests.swift`
...
```swift
// Tests/SemicolynKitTests/TmuxSessionStateTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
@testable import SemicolynKit
...
```swift
// Sources/SemicolynKit/Tmux/TmuxSessionState.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import Foundation
...
```bash
git add Sources/SemicolynKit/Tmux/TmuxSessionState.swift Tests/SemicolynKitTests/TmuxSessionStateTests.swift
...
**Files:**
- Modify: `Sources/SemicolynKit/Tmux/TmuxSessionState.swift` (replace the placeholder cases in `apply`)
- Test: `Tests/SemicolynKitTests/TmuxSessionStateTests.swift` (append cases)
...
```swift
    ...
    func testSessionChangedSetsIdentity() {
        let s = state([.sessionChanged(SessionID(raw: 0), name: "semicolyn-a3f7c2e9")])
        XCTAssertEqual(s.sessionID, SessionID(raw: 0))
        XCTAssertEqual(s.sessionName, "semicolyn-a3f7c2e9")
    }
...
Expected: PASS — all SemicolynKit suites green (prior tests + the new `PaneLayoutPanes` and `TmuxSessionState` suites).
