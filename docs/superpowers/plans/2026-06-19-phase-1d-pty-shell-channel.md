...
- **No fixture changes.** The current Alpine `sshd` already provides `/bin/sh` and `stty`; all tests gate behind `SEMICOLYN_TEST_SSHD`.
...
| File | Responsibility |
|---|---|
| `crates/semicolyn-ssh-core/src/connection.rs` | `ShellOutput` delegate, `ShellExit`, `ShellSession`, `ShellCommand` (internal), `Connection::open_shell`, the `pump` task |
| `crates/semicolyn-ssh-core/tests/shell_integration.rs` | Shell channel integration tests vs `sshd` (test double + condition polling) |
...
- Create: `crates/semicolyn-ssh-core/tests/shell_integration.rs`
...
async fn shell_echoes_written_input() {
    ...
    let session = conn
        ...
        .await
        .expect("open shell");
    session.write(b"echo semicolyn-marker\n".to_vec()).await.expect("write");
    let saw = wait_until(|| col.text().contains("semicolyn-marker")).await;
...
Run: `HOST_UID=$(id -u) HOST_GID=$(id -g) docker compose run --rm dev cargo test -p semicolyn-ssh-core --test shell_integration`
...
In `crates/semicolyn-ssh-core/src/connection.rs`, add after the `AuthOutcome` / `outcome()` block:
```rust
...
In `crates/semicolyn-ssh-core/src/connection.rs`, add a free function (near `connect_core`):
```rust
...
In `crates/semicolyn-ssh-core/src/connection.rs`, add inside the existing `#[uniffi::export(async_runtime = "tokio")] impl Connection` block (alongside the `authenticate_*` methods):
```rust
...
In `crates/semicolyn-ssh-core/src/connection.rs`, add a new exported impl block (separate, so the tokio async-runtime attribute applies):
```rust
...
The `handle` field is now used by auth (1c) and `open_shell` (1d). In `crates/semicolyn-ssh-core/src/connection.rs`, in the `Connection` struct, change:
```rust
...
```bash
git add crates/semicolyn-ssh-core/src/connection.rs crates/semicolyn-ssh-core/tests/shell_integration.rs
...
**Files:**
- Modify: `crates/semicolyn-ssh-core/src/connection.rs`
- Modify: `crates/semicolyn-ssh-core/tests/shell_integration.rs`
...
Add to `crates/semicolyn-ssh-core/tests/shell_integration.rs`:
```rust
#[tokio::test]
...
Run: `HOST_UID=$(id -u) HOST_GID=$(id -g) docker compose run --rm dev cargo test -p semicolyn-ssh-core --test shell_integration shell_resize`
Expected: FAIL — `no method named resize`.

- [ ] **Step 3: Implement `resize`**

In `crates/semicolyn-ssh-core/src/connection.rs`, add inside the `#[uniffi::export(async_runtime = "tokio")] impl ShellSession` block:
```rust
...
Replace the `_uses_connect_error` placeholder in `crates/semicolyn-ssh-core/tests/shell_integration.rs` with:
```rust
#[tokio::test]
...
```bash
git add crates/semicolyn-ssh-core/tests/shell_integration.rs
...
- [ ] `cargo test -p semicolyn-ssh-core` green against the running `sshd` fixture; earlier suites (unit / connect / auth) unaffected.
