...
**Architecture:** The tmux control-mode *engine* (parser, `TmuxSessionController`, session model) already exists and is tested in `SemicolynKit/Tmux/`. This plan adds (1) a small engine change to **surface `%output` pane bytes** (currently dropped), (2) **pure launch-decision logic** in `SemicolynKit` (version parsing, session naming, attach-vs-degrade decision) — all Linux-tested, and (3) the **iOS app glue** that runs the probe over the existing `open_exec` transport, drives the controller, routes the active pane's bytes to the existing single SwiftTerm view, and shows the degraded banner. A Rust integration test against the tmux-equipped CI sshd fixture proves the transport.

**Tech Stack:** Swift 6 `SemicolynKit` (XCTest, Linux), the `SemicolynSSHCoreFFI` UniFFI bridge + SwiftUI app target (macOS-CI compile gate), Rust integration test (`semicolyn-ssh-core`, linux-rust against the `tmux`-equipped sshd fixture).

## Scope

**This plan (Phase 3 Plan A):**
- Connect-time `tmux -V` probe over `open_exec`; decision = attach / degrade, honoring the host's `semicolyn.tmux.attemptControlMode`.
- Attach `tmux -CC new-session -A -s semicolyn-<hash>` (session name via a swappable stub provider).
...
|---|---|---|
| `Sources/SemicolynKit/Tmux/TmuxSessionController.swift` *(modify)* | add `paneOutput` to `TmuxControllerOutput`; collect `.output` events in `feed` | Linux `swift test` |
| `Sources/SemicolynKit/Tmux/TmuxLaunch.swift` *(create)* | pure: `parseTmuxVersion`, `tmuxSupportsControlMode`, `TmuxLaunchDecision`, `tmuxLaunchDecision(...)`, `SessionNameProvider` + `StubSessionNameProvider`, `tmuxSessionName(seed:)` | Linux `swift test` |
| `crates/semicolyn-ssh-core/tests/tmux_integration.rs` *(modify)* | add a `tmux -CC new-session -A` attach assertion | linux-rust (fixture-gated) |
...
- Test commands: `docker compose run --rm dev swift test --filter <Class>`; `docker compose run --rm dev cargo test -p semicolyn-ssh-core` (the tmux attach test is gated behind the existing `SEMICOLYN_TEST_SSHD` env the suite already uses).
...
**Files:**
- Modify: `Sources/SemicolynKit/Tmux/TmuxSessionController.swift`
- Test: `Tests/SemicolynKitTests/TmuxControllerOutputTests.swift` (create)
...
```swift
// Tests/SemicolynKitTests/TmuxControllerOutputTests.swift
...
```bash
git add Sources/SemicolynKit/Tmux/TmuxSessionController.swift Tests/SemicolynKitTests/TmuxControllerOutputTests.swift
...
All the attach-vs-degrade reasoning as pure functions in `SemicolynKit`, so the app layer is thin I/O. Linux-tested.

**Files:**
- Create: `Sources/SemicolynKit/Tmux/TmuxLaunch.swift`
- Test: `Tests/SemicolynKitTests/TmuxLaunchTests.swift`
...
- Produces (consumed by Task 4/5):
  ...
  - `func tmuxSessionName(seed: String) -> String` — `"semicolyn-" + first 8 lowercase hex of SHA-256(seed)`.
...
```swift
// Tests/SemicolynKitTests/TmuxLaunchTests.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import XCTest
@testable import SemicolynKit
...
final class TmuxLaunchTests: XCTestCase {
    ...
    func testSessionNameIsStableHexSlug() {
        ...
        XCTAssertTrue(name.hasPrefix("semicolyn-"))
        let hex = name.dropFirst("semicolyn-".count)
...
```swift
// Sources/SemicolynKit/Tmux/TmuxLaunch.swift
// SPDX-FileCopyrightText: 2026 True Positive LLC
// SPDX-License-Identifier: GPL-3.0-only
import Foundation
...
/// Why Semicolyn fell back to a raw-PTY shell instead of attaching control mode.
...
/// `semicolyn-<first 8 lowercase hex of SHA-256(seed)>`.
public func tmuxSessionName(seed: String) -> String {
    let digest = SHA256.hash(data: Data(seed.utf8))
    let hex = digest.map { String(format: "%02x", $0) }.joined()
    return "semicolyn-" + hex.prefix(8)
}
...
```bash
git add Sources/SemicolynKit/Tmux/TmuxLaunch.swift Tests/SemicolynKitTests/TmuxLaunchTests.swift
...
**Files:**
- Modify: `crates/semicolyn-ssh-core/tests/tmux_integration.rs`

**Interfaces:**
- Consumes (exist): `connect_core`, `Connection.open_exec`, the `Collector` test sink, `TrustAll`, the `SEMICOLYN_TEST_SSHD` gate (read the file's existing helpers/setup first and reuse them).

- [ ] **Step 1: Read the existing test file and its helpers**

```bash
sed -n '1,140p' crates/semicolyn-ssh-core/tests/tmux_integration.rs
```
...
async fn tmux_cc_new_session_produces_control_mode_handshake() {
    let Some(conn) = connect_and_auth().await else { return };  // skips when SEMICOLYN_TEST_SSHD unset
    let collector = Collector::new();
    let _session = conn
        .open_exec(
            "tmux -CC new-session -A -s semicolyn-itest".to_string(),
...
> Match the real helper names in the file (`connect_and_auth`/`wait_until` are illustrative — use whatever the file already defines; if there's no poll helper, loop with `tokio::time::sleep` up to ~5s checking `collector.text()`). Keep the test behind the same `SEMICOLYN_TEST_SSHD` skip guard the other integration tests use, so it no-ops in environments without the fixture.

- [ ] **Step 3: Run the integration test**

```bash
docker compose run --rm dev cargo test -p semicolyn-ssh-core --test tmux_integration
```
...
```bash
git add crates/semicolyn-ssh-core/tests/tmux_integration.rs
...
**Interfaces:**
- Consumes: `TmuxSessionController` (`start`, `submit`, `feed`, `lifecycle`, `state`), `TmuxControllerOutput.paneOutput` (Task 1), `PaneOutputChunk`, `TmuxCommand.sendKeys(target:bytes:)`, `TmuxSessionState.window(_:).activePane` / `state.activeWindow`, `SemicolynSSHCoreFFI.ShellOutput` (the foreign trait `TerminalShellOutput` already implements in `App/Bridges.swift`), `ShellSession.write`.
...
> If the design tokens define an amber/warning color, use it instead of `Color.orange` to match the app's token discipline; check `Sources/SemicolynKit/Theme` for a warning token and substitute. `Color.orange` is the fallback only.
...
git add README.md crates/semicolyn-ssh-core
