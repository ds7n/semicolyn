...
- **Phase 4 — keybar UI (4a #14, 4b #15, 4c #16, 4d-1 done).** Remaining slices: **4d-2** custom-slot binding model + macro model/template-parse + record/template macro creation + full searchable **Launcher** screen + custom-slot editor (the `KeybarSlot` enum + editor "+ Add" sheet are left ready to extend); **4e** external keyboard (`UIKeyCommand` map, hardware modifiers, compact bar). Specs: `2026-06-15-keybar-customization-design.md`, `2026-06-17-external-keyboard-design.md`.
- **Simulator/device pass on the keybar (4a + 4b + 4c + 4d-1)** — compile-only-validated so far. Verify: mount-above-keyboard (`inputAccessoryView` vs the v1 `safeAreaInset`), slot recolor, gestures, predictor-strip slide/chips/tap-complete; **plus the unrendered spec visuals** — promoted-slot bronze top-edge accent (context spec §"Promoted slot visual") and Fn-locked brighter glyph + 4pt lock-dot (function-keys spec §"Visual"). **4d-1 editor**: Esc-pill long-press → Settings→Keybar; verify drag-reorder, swipe/edit-mode delete (Esc/Pad locked), per-row move-across-divider menu, reverse-bar mirror (`layoutDirection` flip), Reset. Note 4d-1 used two sections + a per-row move action instead of the spec's single draggable divider (SwiftUI cross-section drag is unreliable) — revisit. Gated on Apple enrollment for device.
