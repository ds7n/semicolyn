...
**Tests green:** 17 Rust unit + 35 Rust integration (vs containerized `sshd`) + 511 Swift (NeotildeKit + SeedKit), all on the Linux fast loop.
...
| **3 — Terminal + tmux** | Control-mode stack done²; **Plans A+B+C done**: probe→attach `tmux -CC`, native pane layout, multi-window, debounced resize, raw-PTY degrade; terminal UX (bell, OSC 52, titles, URL tap, cursor, mouse dot, pinch-zoom). **Plan D pending** | ◐ Multi-pane + UX live; Plan D pending |
| **4 — Keybar, input & predictor** | Predictor engine done (CMS + Bloom, prefix + bigram, rollover, seed deference, privacy filter, harvesting). **Keybar UI + app-edge wiring pending** | ◐ Engine done, UI pending |
...
- **Phase 3 Plan D** — context state-machine (per-pane foreground-process detection → keybar promotions) + crash banner. Specs: `2026-06-14-context-detection-design.md`.
- **Phase 4 — keybar UI** — Fn layout engine, context-detection promotions, predictor strip, customization, app-edge wiring (the engine is done). Specs: `2026-06-15-keybar-customization-design.md`, `2026-06-14-function-keys-design.md`, `2026-06-17-external-keyboard-design.md`.
