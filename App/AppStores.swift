...
final class AppStores {
    ...
    init() throws {
        ...
        // Install the bundled prose seed the same way, into a separate SeedStore
        // directory so it never collides with the CLI-history seed above. No-op
        // today: the `prose_en_v1.sketch` resource is generated by a later task.
        installBundledProseSeedIfNeeded()
    }
    ...
    /// The bundled/installed prose seed, or nil if none is installed yet (including
    /// when the `prose_en_v1.sketch` resource has not been generated/bundled yet).
    func proseSeed() -> PredictorSeed? {
        SeedStore(directory: baseDirectory.appendingPathComponent("predictor-prose", isDirectory: true)).loadSeed()
    }

    /// Read the bundled prose seed resource and install it via SeedStore
    /// (idempotent + self-healing), mirroring `installBundledSeedIfNeeded` but into
    /// a separate directory so it never collides with the CLI-history seed. No-op
    /// if the resource is absent (the corpus-generation task lands it later) or
    /// unparseable. Never throws into launch.
    private func installBundledProseSeedIfNeeded() {
        guard let url = Bundle.main.url(forResource: "prose_en_v1", withExtension: "sketch"),
              let data = try? Data(contentsOf: url) else {
            DebugLog.shared.log(.seed, "prose-seed:install skipped=no-resource")
            return
        }
        guard let bundled = BundledSeed(combinedBlob: [UInt8](data)) else {
            DebugLog.shared.log(.seed, "prose-seed:install skipped=unparseable bytes=\(data.count)")
            return
        }
        let store = SeedStore(directory: baseDirectory.appendingPathComponent("predictor-prose", isDirectory: true))
        do {
            let didInstall = try store.installIfNeeded(bundled)
            DebugLog.shared.log(.seed, "prose-seed:install installed=\(didInstall) version=\(bundled.version)")
        } catch {
            DebugLog.shared.log(.seed, "prose-seed:install failed error=\(error)")
        }
    }
